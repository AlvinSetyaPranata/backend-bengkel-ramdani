...
export default function DatetimeField({ title, name, setter, defaultValue="" }: Props) {
  ...
  return (
    <View>
      ...
      <View style={styles.inputContainer}>
        <TextInput readOnly={true} value={value.getFullYear().toString()}  style={styles.input} />
        <Pressable onPress={() => setShow(true)}>
          <MaterialIcons name="date-range" color="black" size={16} />
        </Pressable>
      </View>
      {show && <RNDateTimePicker value={new Date()} onChange={onChangeHandler} />
      }
    </View>
  )
}
